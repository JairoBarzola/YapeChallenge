...
import com.jairbarzola.yapechallenge.core.networking.RetrofitHelper
...
class RecipeRepositoryImplTest {
    ...
    fun `for no recipes, api must return empty with http code 200`() = runTest {
        ...
        assertThat((actualResponse as ResultType.Success).value).isEmpty()
    ...
    fun `for multiple recipes, api must return all recipes with http code 200`() = runTest {
        ...
        assertThat((actualResponse as ResultType.Success).value).hasSize(recipes.size)
    ...
    fun `for server error, api must return with http code 5xx`() = runTest {
        ...
        assertThat((actualResponse as ResultType.Error).value).isInstanceOf(Failure.Message::class.java)
    ...
    fun `for recipe id, api must return with http code 200 and user object`() = runTest {
        ...
        assertThat((actualResponse as ResultType.Success).value).isEqualTo(mockUser)
    }
    ...
    fun `for incorrect setup recipes list, method must return an Exception`() = runTest {
        ...
        assertThat((actualResponse as ResultType.Error).value).isInstanceOf(Failure.Exception::class.java)
